...
# 
# dat<-alldat%>% dplyr::select(id, Women,Bottom,Switch, Gender, BDSMRole,Hetero:Urban, Single, Casual, Committed, AGE_SEX, PARTNERS, YR_PRACTICE, YR_COMMUN, ASSERT:YOST_RL, COMMUN, -RCUS)#, -BDSM_1, -College, -Urban, -White)
...
#218 cases remain
predictors<-dat %>% dplyr::select(YOST_Wrong, YOST_V, YOST_LoT, YOST_RL, YR_PRACTICE, YR_COMMUN, COMMUN)
outcomes<-dat %>% dplyr::select(ASSERT, ASSUME, POS, PBC, RLN, IBA, AWARE, LIKELY_COERCE)
```
...
dat<-alldat%>% dplyr::select(id, Bottom,Switch, Gender2, BDSMRole,Hetero:Urban, Single, Casual, Committed, AGE_SEX, PARTNERS, YR_PRACTICE, YR_COMMUN, COMMUNITY_1:COMMUNITY_6, ASSERT:YOST_RL, COMMUN,-RCUS)
...
consent<-df %>% dplyr::select(ASSUME, POS, PBC, RLN, IBA, AWARE, LIKELY_COERCE)
...
consent<-dat %>% dplyr::select(ASSUME, POS, PBC_ln, RLN, IBA, AWARE_ln, LIKELY_ln)
